...
// The plain config lives in firebase-config.js, which imports no SDK, so modules that
// only need `firebaseEnabled` or a public client id can have it without pulling 199 KB
// of Firebase into their chunk. Re-exported here so existing importers are unaffected.
import { firebaseConfig, FUNCTIONS_REGION, firebaseEnabled, GOOGLE_OAUTH_CLIENT_ID, FCM_VAPID_KEY } from './firebase-config.js';

export { firebaseEnabled, GOOGLE_OAUTH_CLIENT_ID, FCM_VAPID_KEY };
