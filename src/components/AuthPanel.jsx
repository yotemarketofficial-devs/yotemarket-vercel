/* AuthPanel.jsx — the ONE unified sign-in / sign-up experience, shared by the
   shopper storefront and the merchant dashboard so both onboard identically
   (same layout, role selector, fields, providers). Wired to real Firebase Auth
   via useAuth. Uses only the shared design tokens (--m-*) + .ym-* classes that
   both kits define, plus a local FontAwesome helper, so it drops into either
   scope with no cross-kit coupling.

   Props:
     overlay      — render as a dismissible overlay (storefront) vs full page (gate)
     onClose      — close handler (overlay)
     theme,onTheme— theme + toggle for the brand panel
     stayRole     — the role that belongs to THIS app and shouldn't redirect
                    ('shopper' in the storefront, 'merchant' in the dashboard)
     defaultRole  — pre-selected role (defaults to stayRole)
     showGuest    — show "Continue browsing as guest"
     onGuest      — guest handler
...
const ROLE_DEST = { shopper: '/storefront', merchant: '/dashboard', rider: '/rider' };

function BrandPanel({ theme, onTheme }) {
  return (
    <div style={{ position: 'relative', overflow: 'hidden', backgroundImage: "linear-gradient(180deg,rgba(26,6,64,.82) 0%,rgba(42,14,100,.6) 55%,rgba(26,6,64,.4) 100%), url('/assets/marketing/banner.png')", backgroundSize: 'cover', backgroundPosition: 'center 40%',
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        ...
        {onTheme && <button onClick={onTheme} className="icon-btn" aria-label="Toggle theme" style={{ background: 'rgba(255,255,255,.16)', color: '#fff' }}><FA i={theme === 'dark' ? 'fa-sun' : 'fa-moon'} /></button>}
      </div>
      <div>
